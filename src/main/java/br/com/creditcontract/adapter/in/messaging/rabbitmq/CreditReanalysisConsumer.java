...
/** Inbound RabbitMQ adapter for at-least-once credit-reanalysis requests. */
@Component
public class CreditReanalysisConsumer {

	private static final Logger LOGGER = LoggerFactory.getLogger(CreditReanalysisConsumer.class);
	private static final String CONSUMER_NAME = "credit-reanalysis";
	private final ProcessCreditReanalysisUseCase useCase;
	private final ObjectMapper objectMapper;
	private final Counter successCounter;
	private final Counter failureCounter;
	private final Timer processingTimer;

	public CreditReanalysisConsumer(
			ProcessCreditReanalysisUseCase useCase,
	...
	@RabbitListener(queues = RabbitMqTopology.CREDIT_REANALYSIS_REQUESTS_QUEUE)
	public void consume(Message message) throws IOException {
		...
		try {
			processingTimer.recordCallable(() -> {
				...
				useCase.execute(new ProcessCreditReanalysisCommand(
			...
			throw new IOException("could not process credit-reanalysis message", exception);
		} finally {
			MDC.remove("eventId");
