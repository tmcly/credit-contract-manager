...
public class CancelCreditContractController {
	...
	@ApiResponses({
			...
			@ApiResponse(responseCode = "409", description = "Current state rejects cancellation or a concurrent command changed it.",
					content = @Content(mediaType = "application/problem+json", schema = @Schema(implementation = ApiProblemResponse.class),
							examples = {
									@ExampleObject(name = "Invalid transition", value = OpenApiExamples.CANCELLATION_CONFLICT_PROBLEM),
