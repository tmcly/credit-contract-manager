...
public class RabbitMqEventPublisher implements EventPublisher {
	...
	private String routingKey(EventPublication event) {
		...
		if (CreditContractActivated.EVENT_TYPE.equals(event.eventType()) && event.schemaVersion() == 1) {
			return RabbitMqTopology.CREDIT_CONTRACT_ACTIVATED_ROUTING_KEY;
		}
		...
		if (CreditContractCancelled.EVENT_TYPE.equals(event.eventType()) && event.schemaVersion() == 1) {
			return RabbitMqTopology.CREDIT_CONTRACT_CANCELLED_ROUTING_KEY;
		}
