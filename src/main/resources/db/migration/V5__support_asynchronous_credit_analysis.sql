...
-- Limits issued by the former synchronous stub did not represent an approved
-- lifecycle state. Existing pre-analysis rows adopt the new invariant.
...
ALTER TABLE credit_contracts
    ...
    ADD CONSTRAINT chk_credit_contract_limit
        CHECK (
            ...
            OR (status IN ('APPROVED', 'ACTIVE', 'BLOCKED') AND credit_limit > 0)
            OR status = 'CANCELLED'
        );
